/*
    ...
    전역변수 : 시스템상에서 모두 접근할 수있는 변수 (window객체에 등록되는 변수)
    지역변수 : 특정 코드 블록 안쪽에서 선언된 변수 : 선언된 코드블록 안쪽에서만 접근 가능

    전역변수의 오염 : window객체에 등록된 변수는 다른 스크립트 파일이라고 하더라도 해당 변수값을 공유하므로
    실수로 A.js에서 B.js에 있는 변수를 변형 시킬 수 있는 이슈

    script 연결 시 type = 'module'이라고 지정하면 해당 파일 안쪽의 선언된 변수는 설사 코드블록 밖에서 선언되었더라도
    파일자체의 전용 스코프가 설정되어 다른 파일에서 해당 파일의 변수 접근이 불가능한 모듈화 상태 유지

    type = 'module' 지정시 defer와 마찬가지로 스크립트와 body 안쪽 코드가 비동기적으로(동시에) 해석됨
*/
...
function getTime() {
    ...
    if (hr >= 13) {
        ...
        pm.classList.add("on");
        // 13시가 넘어설 때 12를 뺀 값을 다시 hr변수에 재할당
        hr = hr - 12;
    } else {
